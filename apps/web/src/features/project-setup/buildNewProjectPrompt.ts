...
export function buildNewProjectPrompt(project: Project, appOrigin: string): string {
  ...
  return [
    ...
    "",
    "【タスク詳細（WBS 葉タスク）】",
    "- 各ワークパッケージに description（作業内容）を必ず付与する。アプリのタスク詳細ページに表示される。",
    "- description には成果物・判断基準・確認ポイントなど、担当者が何をすべきか分かる具体的内容を書く。",
    "- assignee（担当）は任意。未設定でもよいが、推測できる場合は関係者名や役割を付与する。",
    "",
